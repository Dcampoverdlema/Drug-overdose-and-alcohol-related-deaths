...
```{r}
#Jodie add graph here!!! >:(
```

add your description keep it formal >:(
...
test_pred_all |>
  ...
  stat_summary(fun.data = mean_se, geom = "errorbar", 
               color = "red", width = 0.2) + 
  theme_bw() +
  labs(title="Train and Test: Drug Overdose Deaths", 
...
In **Figure 4**, Random Forest model had the lowest RMSE, which meant it was the most accurate to find our predictors
...
In **Figure 5**,the linear model has the least amount of error compared to the others but could make an argument to use other models like random forest to find predictors. 
...
```
By looking at **Figure 6**, Unemployment is the top predictor for alcohol impaired driving deaths. Then if everything is constant, if unemployment rate increases by 1%, alcohol impaired driving deaths is projected to increase by 1.3%. Other significant predictors are Excessive Drinking, Suicides, and Population Density.  
...
By looking at **Figure 7**, finish here jodie 
...
plot_unemployment <- plot_unemployment + theme_bw() + geom_line(color = "red") + labs(x="Unemployment Rate")
plot_adult_smoking <- plot_adult_smoking + theme_bw()+ geom_line(color = "red") + labs(x="Adult Smoking Rate")
