...
From the research, the question we are going to answer, is there demographic and social factors that are predictors of drug overdose and alcohol-related incidents (e.g., driving accidents)?
...
Lets take a deeper look into Montana Alcohol imparaierd driving deaths by counties: 
...
The graph shows the association, broken down by income level, between unemployment rates and the proportion of alcohol-related driving deaths in Montana counties. To provide a more accurate depiction, the data only includes counties with a population of more than three thousand people.

**Three income levels are represented by the colors of the counties:** 

Low (blue), Average (red), and High (midnight blue). Counties are represented as points in the scatter plot, and each income level is represented by a smoother trend line. Plotting the percentage of driving deaths caused by alcohol uses the y-axis, while the x-axis represents the unemployment rate.

**The trend lines show different trends according to income levels:**

Higher unemployment rates are typically accompanied by a declining trend in alcohol-related driving fatalities in high-income areas (midnight blue).

Counties with average incomes (red) display a more intricate and dynamic pattern, indicating a correlation between moderate unemployment rates and an increased number of alcohol-related driving deaths.
Low-income counties (blue) have a distinctive pattern in which a considerable decrease in alcohol-related driving-related fatalities is correlated with greater unemployment rates, and vice versa.
In order to put certain data points in context, some points are additionally labeled with the median household income (in thousands).
