...
ggplot(data = merged_data, aes(x = long, y = lat, group = group)) +
  ...
  theme(
    ...
    axis.text = element_blank(),
    axis.ticks = element_blank(),  
    plot.title = element_text(hjust = 0.5, face = "bold", size = 15),  # Center, bold the title, and increase font size
...
Just by observation, you can see that West Virginia has the highest amount of drug overdose deaths. By conducting further research on West Virginia, we found that in 2023, they led the nation in opioid related drug overdose deaths. According to the Drug Enforcement Administration, West Virginia has one of the highest prescription rates for opioids in the country, and the state also has a high prevalence of controlled prescription drug addiction and trafficking. Furthermore, the usage of illegal prescription drugs was a factor in almost 61% of state overdose deaths in 2015. 

Then for alcohol impaired driving deaths, Montana had the highest amount of deaths that resulted from alcohol impaired driving. 

By the predictors of both drug overdose deaths and alcohol impaired driving deaths, we can hopefully come up with some solutions and or regulations to potentially help these states with their ongoing problems. 
