...
health_data_new <- read.csv("data/health_data_selected.csv") |>  
  ...
  select(alcohol_impaired_driving_deaths, unemployment, median_household_income, children_in_single_parent_households,social_associations, drug_overdose_deaths, disconnected_youth, homicides, suicides,percent_below_18_years_of_age, percent_65_and_older, percent_native_hawaiian_or_other_pacific_islander, percent_non_hispanic_black, percent_hispanic, percent_female, -living_wage, population)
...
health_data_alc <- health_data_new |> 
  filter(population > 3000) |> 
  mutate(across(everything(), ~ifelse(is.na(.), mean(., na.rm = TRUE), .)))

x <- as.matrix(health_data_alc |>  select(-alcohol_impaired_driving_deaths))
y <- health_data_alc$alcohol_impaired_driving_deaths
...
tidy_lasso_coef %>%
  ...
  labs(title = "Lasso Regression for Alcohol Impaired Driving Deaths",
...
**It seems that percent American Indian and Alaska Native is the top predictor for alcohol impaired driving deaths** 

**Now the same thing for drug overdose deaths ** 


```{r}
x <- as.matrix(health_data_new |>  select(-drug_overdose_deaths))
y <- health_data_new$drug_overdose_deaths
...
tidy_lasso_coef %>%
  ...
  geom_col(color = "white", show.legend = FALSE) +
  scale_fill_manual(values = c("darkred", "darkblue")) +
  labs(title = "Lasso Regression for Drug Overdose Deaths",
       x = "Coefficient Estimate",
       y = "Predictor")
