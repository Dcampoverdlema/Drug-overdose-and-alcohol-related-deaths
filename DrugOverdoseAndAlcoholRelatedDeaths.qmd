...
To confirm the finding in the linear model, we need to look at the p-values before we decide to draw any conclusions. 

First p-values for alcohol impaired driving deaths:

```{r}

model_alc <- lm(formula=alcohol_impaired_driving_deaths ~ ., data=health_data_new)
summary(model_alc)


```

From the linear model, 65 and older was the highest predictor. And this is confirmed the the p-value being very significant (5.34e-14)

Now looking at the p-values for drug overdose deaths: 

```{r}
model_drug <- lm(formula=drug_overdose_deaths ~ ., data=health_data_new)
summary(model_drug)
```

These p-values also confirm the findings from our linear regression model and now we can make some conclusions and answer our question. 
