...
```{r}
# Load the datasets
MT_count_data <- read.csv('../data/MT_count_data.csv') |> 
...
MT_count_data <- kNN(MT_count_data, k = 5, imp_var = FALSE)

# Check for any remaining missing values
sum(is.na(MT_count_data))
...
for (model in models) {
  if (model == "ridge" || model == "lasso" || model == "enet") {
    alpha <- switch(model,
                    ...
                    "lasso" = 1,
                    "enet" = 0.5)
    fit <- train(alcohol_impaired_driving_deaths_raw_value ~ ., data = MT_count_data,
                 method = "glmnet",
...
tree_model <- rpart(alcohol_impaired_driving_deaths_raw_value ~ ., data = MT_count_data, method = "anova")
...
rf_model <- randomForest(alcohol_impaired_driving_deaths_raw_value ~ ., data = MT_count_data, importance = TRUE)
print(rf_model)
