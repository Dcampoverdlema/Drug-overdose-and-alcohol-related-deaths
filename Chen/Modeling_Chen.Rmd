...
## Create graphs comparing the performances 

```{r cars}
# drugs_df2 <- new_drugsdf |>
#   mutate(employment = 1-unemployment)
new_drugsdf <- new_drugsdf |>
  select(Drug_Overdose_Deaths, Unemployment_Rate, Income_Inequality, Median_House_Income, Adult_Smoking_Rate, College_Education_Rate, Social_Associations, Female_Pop, urban_level) |>
  drop_na()
...
fixed <-lm(Drug_Overdose_Deaths~Unemployment_Rate + Income_Inequality + Median_House_Income + Adult_Smoking_Rate + College_Education_Rate + Social_Associations + Female_Pop, data=new_drugsdf)
